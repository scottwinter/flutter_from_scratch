...
class DatabaseHelper {
  ...
  final String columnUpdatedDate = 'updatedDate';
...
//    await deleteDatabase(path); // just for testing
  ...
  void _onCreate(Database db, int newVersion) async {
    await db.execute(
        'CREATE TABLE $tableNote($columnId INTEGER PRIMARY KEY, $columnTitle TEXT, $columnBody TEXT, $columnUpdatedDate INTEGER)');
  }
...
//        'INSERT INTO $tableNote ($columnTitle, $columnDescription) VALUES (\'${note.title}\', \'${note.description}\')');
  ...
  Future<List> getAllNotes() async {
    var dbClient = await db;
    var result = await dbClient.query(tableNote, columns: [columnId, columnTitle, columnBody, columnUpdatedDate], orderBy: "$columnUpdatedDate DESC");
